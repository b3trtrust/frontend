import { X, ChevronRight, Gavel, Wallet } from "lucide-react"
import { Link } from "react-router-dom"
import Logo from "@/components/Logo"
import { arbitratorSpecialties } from "@/data/arbitratorSpecialties"
...
const ArbitratorsSidebar = ({ open, onClose, selectedSpecialty, onSelectSpecialty }: Props) => {
      ...
      <div className="px-4 flex-1 overflow-y-auto">
        ...
        <div className="flex flex-col gap-1">
          ...
          >
            ...
            <span className={`text-xs px-2 py-0.5 rounded-full ${!selectedSpecialty ? "bg-white/20 text-white" : "bg-orange-100 text-orange-500"}`}>
              {MOCK_ARBITRATORS.length}
            </span>
          </button>

          {arbitratorSpecialties.map(({ label, icon: Icon, count }) => (
            <button
              key={label}
              onClick={() => onSelectSpecialty(label === selectedSpecialty ? null : label)}
              className={`flex items-center justify-between px-3 py-2.5 rounded-xl text-sm transition-colors ${
            ...
            >
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full ${selectedSpecialty === label ? "bg-white/20 text-white" : "bg-orange-100 text-orange-500"}`}>
                {count}
              </span>
            </button>
          ))}
...
export default ArbitratorsSidebar
